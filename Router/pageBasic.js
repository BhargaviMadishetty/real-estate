...
Routes1.patch("/generalinfo", async (req, res) => {
    ...
    try {
        ...
        const data = await page.findByIdAndUpdate(req.body._id, { name, postedBy, featuredPackage, contact, saleType, ppdPackage, imageUrl
        }, { new: true }
        )
        console.log(data);
        res.status(200).json({
...
Routes1.patch("/propertydetails", async (req, res) => {
    ...
    try {
        ...
        const data = await page.findByIdAndUpdate(req.body._id, {length, breadth, totalArea, areaUnit, noOfBHK, noOfFloors, attached, westernToilet, furnished, 
